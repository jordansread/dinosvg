render_side <- function(svg, object, side.name){
  ...
  render_axis(g.side, side=side.num, at=side$axis$at, labels=side$axis$labels, lim=lim, view.bounds = view.bounds, tick.len = 5, axis.label=side$label, las=side$axis$las)
