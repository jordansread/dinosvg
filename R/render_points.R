render_points <- function(g.view, x, y, pch=par("pch"), col=par("col"), bg="#FFFFFF00", cex=1, lwd=par("lwd"), xlim, ylim, hovertext=NULL, ...){
  ...
  for (i in seq_len(length(coords$x))){
    if (!is.null(hovertext)){
      ...
      hover.args <- NULL
    points_node(g.geom, coords$x[i], coords$y[i], pch, col, bg, cex, lwd, hover.args)
  }
}
...
points_node <- function(g, x, y, pch, col, bg, cex, lwd, ...){
  ...
  geoms <- list(circle = c(cx=x, cy=y, r=as.crd(cex*std.rad)),
                ...
                up.tri = c(d=as.path(x=c(num[1],num[1]+std.rad*1.125,num[1]-std.rad*1.125), 
                ...
                dwn.tri = c(d=as.path(x=c(num[1],num[1]+std.rad*1.125,num[1]-std.rad*1.125), 
                                      y=c(num[2]+std.rad*1.5,num[2]-std.rad*0.75,num[2]-std.rad*0.75))),
                tri.square = c(d=paste(as.path(x=c(num[1]-std.rad,num[1]+std.rad,num[1]+std.rad,num[1]-std.rad), 
                                        y=c(num[2]-std.rad,num[2]-std.rad,num[2]+std.rad,num[2]+std.rad)),
                                as.path(x=c(num[1],num[1]-std.rad,num[1]+std.rad), y=c(num[2]-std.rad,num[2]+std.rad+num[2]-std.rad)),sep=' ')))
  ...
  node <- switch(pch,
          ...
          '23' = svg_node('circle', g, c(cx=x, cy=y, r=as.crd(cex*2.7), fill=transparent, 'fill-opacity'="0", stroke=col, ...)),
          '24' = svg_node('circle', g, c(cx=x, cy=y, r=as.crd(cex*2.7), fill=transparent, 'fill-opacity'="0", stroke=col, ...)),
          '25' = svg_node('circle', g, c(cx=x, cy=y, r=as.crd(cex*2.7), fill=transparent, 'fill-opacity'="0", stroke=col, ...)),
